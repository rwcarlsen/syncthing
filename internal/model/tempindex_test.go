...
func TestModelTempIndex(t *testing.T) {
	...
	m.Index(device1, "x", files, protocol.FlagIndexTemporary, nil)
	...
	m.Index(device2, "x", files, protocol.FlagIndexTemporary, nil)
	...
	m.Index(device1, "x", files[1:], protocol.FlagIndexTemporary, nil)
...
func TestRequestTempIndex(t *testing.T) {
	...
	// TODO: Why does the temp index arrive first?
	if len(fc.indexes) != 2 || fc.indexes[0].Flags&protocol.FlagIndexTemporary == 0 {
		t.Errorf("Did not receive temp index or index had a wrong flag")
	}

	blk := fc.indexes[0].Files[0].Blocks[0]
